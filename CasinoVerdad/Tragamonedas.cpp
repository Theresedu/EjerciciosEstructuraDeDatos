...
Tragamonedas::Tragamonedas() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        iconos[i].setScale(0.5f, 0.5f); // Ajuste de tamaño para que encajen bien en pantalla
        iconos[i].setPosition(180 + (i * 180), 320);
    }
...
void Tragamonedas::jugar() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        iconos[i].setTexture(iconosTexture[i]);
        iconos[i].setScale(0.5f, 0.5f); // Ajuste para mantener el tamaño correcto
    }
...
void Tragamonedas::render(sf::RenderWindow &window) {
    ...
    for (int i = 0; i < 3; i++) {
        window.draw(iconos[i]); // Dibuja las imágenes de los símbolos
    }
    window.draw(mensaje);
}

bool Tragamonedas::salir(sf::Event event) {
