...
void Blackjack::handleInput(sf::Event event) {
    if (event.type == sf::Event::KeyPressed && !gameOver) {
        if (event.key.code == sf::Keyboard::H) {  // Hit
            dealCard(playerHand);
            checkGameState();
        } else if (event.key.code == sf::Keyboard::S) {  // Stand
            while (calculateScore(dealerHand) < 17) {
                dealCard(dealerHand);
            }
            checkGameState();
        } else if (event.key.code == sf::Keyboard::F) {  // Fold
        ...
        resetGame();  // Permitir reiniciar la partida con la tecla 'Q'
    }
}
...
/*
void Blackjack::setSaldo(double saldo) {
    this->saldo = saldo;
